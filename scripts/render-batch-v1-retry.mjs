#!/usr/bin/env node
// v1 retry — re-render the 5 SKUs that failed/marginal in pass 1 of SKA-40:
//   bpc-157            : "Compouion 7C8A78" gibberish (Imagen leaked hex literal)
...
// Output appends to docs/brand/imagery/renders/v1/{raw,web}/ (overwrites the
// failed files for these 5 labels). Pass-1 clean renders (cjc, mots-c, tb-500)
// are NOT touched.
...
function buildPrompt({ compound, dosage, lot }) {
  ...
  return [
    ...
    `Strictly do not insert any extra alphanumeric code, batch identifier, sample number, hex color value, or any other text line between the compound name and the dosage line. The label is exactly: wordmark, compound, dosage, lot. Nothing else. No invented brand marks, no monogram, no C-logo, no circle-logo, no ® symbol, no ™ symbol, no copyright symbol, no decorative side text, no rotated wraparound text on the side of the vial.`,
